...
vector<complex<double>> Sx={0,1/sqrt(2),0,1/sqrt(2),0,1/sqrt(2),0,1/sqrt(2),0},Sy={0,-i/sqrt(2),0,i/sqrt(2),0,-i/sqrt(2),0,i/sqrt(2),0},Sz={1,0,0,0,0,0,0,0,-1}, I={1,0,0,0,1,0,0,0,1};
...
vector<complex<double>> CN(vector<complex<double>> v, vector<complex<double>> H, double dt){
    ...
    vector<complex<double>>  A=add_M(I,MS(D,H)), phi={0,0,0}, psi=MS(-1,v);
...
int main(){
    /*ofstream field_sph, proj, insproj;
    field_sph.open("Bsph.txt"); field_sph << setprecision(9) << scientific;
    proj.open("proj.txt"); proj << setprecision(9) << scientific;
    insproj.open("insproj.txt"); insproj << setprecision(9) << scientific;
    ...
    //Time constants
    double Rp=2e-3, Tf, dt, b_max=0.7e-3, l;
    //Cross Sections
    double ud=0, uz=0, dz=0;
    //Direction of movement and orthogonal vectors.
    vector<double> e1, e2, temp;
    //Time evoution counter
    int k; 
    cout << "fine" << endl;
    for(int m=0; m<N_Mc; m++){
        ...
        e2 = cross(e1,temp);
        //Initial position
        xo = add_M(MS(1.5*Rp,temp),{1/(sqrt(2)*w),0,0.5*(1/w)});
        ...
        dt=(2/2e2)*(h/(sqrt(real(inner(B,B)))*mue*2*PI));
        k= -2*Tf/dt;
        cout << 2*k << endl;
        ...
        while(k<=2*Tf/dt){
            ...
            //if((k%1000)%150==0){field_sph << k*dt <<" "<< BM <<" "<< th <<" "<< ph << endl;}
            b=MS(1/sqrt(real(inner(B,B))),B);
            ...
            //if((k%1000)%150==0){proj << k*dt <<" "<< norm(inner(psihCN,psihf)) <<" "<< norm(inner(psihCN, psihf2)) <<" "<< norm(inner(psihCN, psihf3)) << endl;}
            //Update of the projections
            Sph=Sp1(b);
            //Update of the Hamiltonians
            HCNh=Hamil(B);
            //Time evolution of the states
            //if((k%1000)%150==0){insproj <<k*dt<<" "<< real(obs(Sph,psihCN))<< endl;}
