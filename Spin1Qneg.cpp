...
int main(){
    ...
    double Bo=8595.766069, Bq=10, Bc=-1e-3;
    double bet=(2./3.)*Bo/Bq, g=(1/6.)*Bc/Bq; 
    ...
    //Time constants
    double Rp=4e-3, Tf, dt, b_max=0.5e-4, l, uu=0, ud=0, uz=0, dd=0, dz=0, zz=0;
    ...
    for(int m=0; m<N_Mc; m++){
        ...
        //Initial position
        if(Bc<0){
            xo = add_M(MS(1.5*Rp,temp),{0,0,sqrt(-g)});
            //cout << sqrt(-g) << endl;
        }
        ...
        dt=(0.01)*(h/(sqrt(real(inner(B,B)))*mue*2*PI));
        k= -2*Tf/dt;
        ...
        uu+=norm(inner(psihCN,psihf3));
        ud+=norm(inner(psihCN,psihf));
        uz+=norm(inner(psihCN,psihf2));
        zz+=norm(inner(psiCN2,psihf2));
        dz+=norm(inner(psiCN2,psihf));
        dd+=norm(inner(psiCN3,psihf));
/*
    ...
    cout <<"Cross section (ud): " << b_max*b_max*PI*ud/N_Mc << endl;
    cout <<"Cross section (uz): " << b_max*b_max*PI*uz/N_Mc << endl;
    cout <<"Cross section (dz): " << b_max*b_max*PI*dz/N_Mc << endl;
