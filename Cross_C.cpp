...
vector<complex<double>> B_field(vector<double> x, double Bc){
    ...
    r.at(0)=Bc*(-48*pow(x.at(2),2)*x.at(0)+12*x.at(0)*pow(x.at(1),2)+12*pow(x.at(0),3));
    r.at(1)=Bc*(-48*pow(x.at(2),2)*x.at(1)+12*x.at(1)*pow(x.at(0),2)+12*pow(x.at(1),3));
    r.at(2)=Bc*(32*pow(x.at(2),3)-48*x.at(2)*pow(x.at(0),2)-48*x.at(2)*pow(x.at(1),2));
    return r;
}
//-------------------------------------------Time Steps-----------------------------------------------------
...
int main(){
    double dVo=7;
    ofstream out;
    out.open("outc.txt"); out << setprecision(9) << scientific;
    cross(1,out);
    for(int i=11; i<=20; i++){
        cout << i << endl;
        cross(i*dVo,out);
