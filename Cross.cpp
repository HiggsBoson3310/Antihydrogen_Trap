...
int cross(double Vo, ofstream &out){
    ...
    //Output files
    /*ofstream  temp_f, field, pos, speed;
    pos.open("pos1.txt"), field.open("field1.txt"), temp_f.open("temp1.txt");
    pos<< setprecision(5) << scientific;
    field<< setprecision(5) << scientific;
    temp_f << setprecision(5) << scientific;
    speed.open("speed1.txt");
    speed << setprecision(5) << scientific;*/
    //Imaginary unit
    complex<double> i(0,1);
    ...
    //Time constants
    double Rp=2e-3, Tf, dt, b_max=0.1e-3, l, uu=0, ud=0, uz=0, dd=0, dz=0, zz=0;
    ...
    for(int m=0; m<N_Mc; m++){
        ...
        e2 = cross(e1,temp);
        //Initial position
        xo = add_M(MS(1.5*Rp,temp),{1/sqrt(2)*1/((2./3.)*Bo/Bq),0,0.5*1/((2./3.)*Bo/Bq)});
